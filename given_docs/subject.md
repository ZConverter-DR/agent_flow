...
Target Server가 없다는 가정하에 진행되는 recover_server 그래프 구현 중

1. 복구에 필요한 openstack mcp server의 tool을 Mock에서 실제 openstack 연동을 구현 중
2. get_server, create_vm 과 같은 sdk 호출할 때, 가장 기본적인 데이터만 주고 제대로 구동되는 것이 1차 목표
3. 실제 복구 서비스 로직을 파악하고, 추가로 필요한 데이터를 수집 및 VM 생성에 적용

Target Server가 없다는 가정하에 진행되는 recover_server 그래프 구현 중
Phase 1 - LangGraph graph 로직 구현
Phase 2 - OpenStack SDK 연동 / RAG 플로우 구현
Phase 3 - ZCoverter 에이전트를 Target VM에 설치 후 동작?
Phase 4 - 각 단계별 플로우 고도화

- 각 SDK 로직 고도화, 필요 데이터 보완
- invoke로 출력되고 있는 것들 stream으로 구현
- Redis key
  Phase 5
- 상황별 agent graph 추가 구현
- 감사 로깅 시스템 구현
  Phase 6
-
